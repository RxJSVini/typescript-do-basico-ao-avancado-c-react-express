...
const inputEmail = document.getElementById('inputMail') as HTMLInputElement;
...
class Produto {
    ...
    protected validaDesconto(preco: number, desconto: number){
        if(desconto){
...
class Pessoa {
    constructor(private nome: string, public idade: number, private altura:number){
        this.nome;
        this.idade
        this.altura = altura;
    }
...
class PessoaFuncionario extends Pessoa {
    constructor(nome: string, idade: number, altura: number, genero: "m" | "f" ){
...
class Proprietario {
    ...
    public set(propriedade: string | string[]): void {
...
abstract class CalculoAbstrato {
...
class Unico {
    ...
    agora(){
        ...
        return response.toLocaleTimeString(
            'pt-Br',
